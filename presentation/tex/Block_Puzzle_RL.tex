...
\title{Block Blast!: Обучение с подкреплением}
\subtitle{MaskablePPO · 4 поколения агентов · MLP + CNN + ViT · BC+PPO}
\date{2026}
...
\begin{document}
	...
	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Поколение 4: масштабирование на 16×16}
		\begin{columns}[t]
			\begin{column}{0.50\textwidth}
				\textbf{Почему 16×16 должно быть интереснее?}

				\medskip
				\begin{itemize}
					\item Эвристика остаётся одношаговой жадной — цена ошибки выше
					\item RL-агент оптимизирует глобальный сигнал — нелокальные стратегии достижимы
					\item Сложность эвристики: $O(n^4)$ — при росте поля растёт вдвое быстрее
					\item Один форвард нейросети не зависит от $n$
				\end{itemize}
			\end{column}
			\begin{column}{0.46\textwidth}
				\textbf{Что изменилось технически}

				\medskip
				\begin{itemize}
					\item Пространство действий: $3\times16\times16 = 768$
					\item Тензор наблюдений: $(15, 16, 16)$ — добавлен канал~14: \emph{Holes-from-border mask}
					\item SmallCNN заменён на HierarchicalCNN
					\item Добавлена архитектура SmallViT для сравнения
				\end{itemize}
			\end{column}
		\end{columns}
	\end{frame}
	...
	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Проблема холодного старта на 16×16}
		\begin{columns}[t]
			\begin{column}{0.50\textwidth}
				\textbf{Почему прямой PPO не работает?}

				\medskip
				\begin{itemize}
					\item На $8\times8$ случайный агент очищает $\approx1.8$ lines/эп. — достаточно для PPO
					\item На $16\times16$ тот же случайный агент: $\approx0.5$ lines/эп.
					\item Сигнал слишком слаб: advantage $\hat{A}_t$ вычисляется по случайному шуму
					\item Прямой PPO застревает ниже 1 line/эп. неопределённо долго
				\end{itemize}
			\end{column}
			\begin{column}{0.46\textwidth}
				\textbf{Решение — Behavior Cloning как тёплый старт}

				\medskip
				Обучить политику имитировать эксперта (HeuristicAgent) через supervised learning:

				\[
					\mathcal{L}_{\text{BC}}(\theta) = -\frac{1}{N}\sum_{i=1}^{N} \log \pi_\theta\!\left(a^{(i)}_{\text{exp}} \mid s^{(i)}\right)
				\]

				\medskip
				$N = 500\,000$ пар $(s, a_{\text{exp}})$ — собраны от HeuristicAgent
			\end{column}
		\end{columns}
	\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Behavior Cloning: двухфазный пайплайн}
		\begin{columns}[t]
			\begin{column}{0.50\textwidth}
				\textbf{Фаза 1 — Behavior Cloning}

				\medskip
				\begin{itemize}
					\item Датасет: 500\,000 пар $(s, a_{\text{exp}})$ от HeuristicAgent
					\item 50 эпох, batch\_size=512, $\eta=10^{-3}$ + cosine annealing
					\item Top-1 accuracy: \textbf{40--45\%}
					\item Низкое значение — эвристика случайно выбирает среди равноценных ходов (tiebreak)
					\item После фазы 1: политика стартует с $\approx4$ lines/эп.
					\item Время на Kaggle T4: $\approx1$ ч.
				\end{itemize}
			\end{column}
			\begin{column}{0.46\textwidth}
				\textbf{Фаза 2 — PPO fine-tuning}

				\medskip
				\begin{itemize}
					\item Актор загружается из BC-чекпоинта
					\item Критик инициализируется \emph{случайно} — BC не оптимизирует RL-объектив
					\item 15М шагов MaskablePPO, VecNormalize (\texttt{norm\_reward=True})
					\item $\eta: 5\times10^{-5} \to 5\times10^{-6}$ (линейный schedule)
					\item Время: $\approx7$ ч. на T4
				\end{itemize}
			\end{column}
		\end{columns}
	\end{frame}
	...
	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{HierarchicalCNN для доски 16×16}
		SmallCNN на $16\times16$: рецептивное поле $7\times7$ покрывает лишь $\approx19\%$ доски — заменён на HierarchicalCNN с понижением разрешения.
		...
		\begin{center}
			\begin{tabular}{@{}lll@{}}
				\toprule
				\rowcolor{ThemeGridStroke}
				\color{ThemeCellFilled}Блок & \color{ThemeCellFilled}Выход & \color{ThemeCellFilled}Примечание \\
				\midrule
				Conv(15→32, 3×3) + BN + ReLU  & [B, 32, 16, 16] & stride=1, padding=1 \\
				Conv(32→64, 3×3, stride=2) + BN  & [B, 64, 8, 8]   & ×2 уменьшение \\
				Conv(64→128, 3×3, stride=2) + BN & [B, 128, 4, 4]  & ×4 уменьшение \\
				Conv(128→256, 3×3, stride=2) + BN & [B, 256, 2, 2] & ×8 уменьшение \\
				Flatten → Linear → ReLU          & [B, 512]        & компактный эмб. \\
				Actor [512→256] / Critic [1024→512] & —             & \\
				\bottomrule
			\end{tabular}
		\end{center}

		\bigskip
		Параметров: $\approx1.29$М. BatchNorm допустим благодаря BC warm-start — статистики BN устойчивы с первых шагов PPO.
	\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{SmallViT: трансформерная архитектура}
		\begin{columns}[t]
			\begin{column}{0.50\textwidth}
				\textbf{Архитектура}
				...
				\medskip
				\begin{enumerate}
					\item \textbf{Patch embedding:} поле $16\times16$ делится на $16$ патчей $4\times4$; каждый проецируется в $\mathbb{R}^{256}$
					\item \textbf{CLS-токен + позиционные эмбеддинги} — обучаемые
					\item \textbf{2 слоя TransformerEncoder}, Pre-LN, 4 головы, FFN $256\to512\to256$
					\item \textbf{Агрегация:} патчи усредняются, LayerNorm, Linear($256\to512$)
				\end{enumerate}

				\medskip
				Параметров: $\approx1.25$М
			\end{column}
			\begin{column}{0.46\textwidth}
				\textbf{Преимущество трансформера}

				\medskip
				Само-внимание позволяет любому патчу взаимодействовать с любым другим вне зависимости от расстояния:

				\smallskip
				«Размещение фигуры в левом углу влияет на выживаемость позиции в правом» — нелокальная зависимость, которую CNN улавливает медленнее.
			\end{column}
		\end{columns}
	\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Кривые обучения Gen 4}
		\begin{center}
			\includegraphics[height=0.75\textheight,keepaspectratio]{chart_learning_gen4}
		\end{center}
	\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Результаты Gen 4: доска 16×16}
		\begin{center}
			\includegraphics[height=0.82\textheight,keepaspectratio]{chart_comparison_gen4}
		\end{center}
	\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{Диагональная стратегия}
	
	\begin{center}
		\includegraphics[width=0.75\textwidth,keepaspectratio]{fig_diagonal_step30}
	\end{center}
	...
	Все нейросетевые агенты заполняют поле \textbf{по диагонали из левого верхнего угла}.
	
\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{Статистический анализ Gen 4}
	
	\footnotesize
	500 эпизодов · seed=42 · Mann--Whitney + Bootstrap CI
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}{0.58\textwidth}
			...
			\begin{tabular}{@{}lcc@{}}
				\toprule
				\rowcolor{ThemeGridStroke}
				\color{ThemeCellFilled}\textbf{Метрика} &
				\color{ThemeCellFilled}\textbf{CNN} &
				\color{ThemeCellFilled}\textbf{ViT} \\
				\midrule
				$p$-value (MW)           & $<0.001$            & $<0.001$ \\
				CLES                     & $0.852$             & $0.847$ \\
				Cohen's $d$              & $\mathbf{+1.39}$    & $\mathbf{+1.34}$ \\
				Bootstrap 95\% CI        & $[10.6;\,12.7]$     & $[10.1;\,12.2]$ \\
			...
			\begin{itemize}
				\item $d=1.39$ соответствует \textbf{очень большому эффекту}
				\item CLES $=0.852$: Оба выигрывают примерно в \textbf{85\%} пар эпизодов
				\item CNN vs ViT: $p=0.42$ — различие архитектур статистически незначимо
			\end{itemize}
		...
		\begin{column}{0.40\textwidth}
			...
			\begin{tabular}{@{}lccc@{}}
				\toprule
				\rowcolor{ThemeGridStroke}
				\color{ThemeCellFilled}\textbf{Порог} &
				\color{ThemeCellFilled}\textbf{CNN} &
				\color{ThemeCellFilled}\textbf{ViT} &
				\color{ThemeCellFilled}\textbf{Heur.} \\
				\midrule
				$\geq 1$ line   & 100\% & 100\% & 88\% \\
				$\geq 10$ lines & 85\%  & 84\% & 32\% \\
				$\geq 20$ lines & 41\%  & 41\% & 7\% \\
				$\geq 30$ lines & 13\%  & 13\% & 1\% \\
				\bottomrule
			\end{tabular}
			...
			\begin{itemize}
				\item CNN и ViT очищают хотя бы одну линию в \textbf{100\%} эпизодов
				\item При пороге $\geq30$ линий превосходство достигает \textbf{13$\times$}
			\end{itemize}
...
\end{frame}
	...
	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Survival Analysis Gen 4}
		\begin{center}
			\includegraphics[height=0.82\textheight,keepaspectratio]{chart_survival_gen4}
		\end{center}
	\end{frame}

	% ──────────────────────────────────────────────────────────────────────────────
	\begin{frame}{Финальное сравнение: все поколения}
		\begin{center}
			\includegraphics[height=0.75\textheight,keepaspectratio]{chart_lines_final}
		\end{center}
	\end{frame}
	...
	\begin{frame}{Итоговые выводы}
		\begin{itemize}
			\item \textbf{Нейросеть превзошла вручную оптимизированный алгоритм} без единой строки знаний о правилах: на $8\times8$ — $+32\%$, на $16\times16$ — \textbf{$2.6\times$} по lines (Cohen's $d=1.39$).

			\item \textbf{Инженерия наблюдений — ключевой фактор:} каждое поколение превзошло предыдущее при неизменном алгоритме PPO. Gen~1→2: $+12\%$, Gen~2→3: $+13\%$.

			\item \textbf{Transfer Learning и Behavior Cloning решают проблему холодного старта:} CNN не обучается с нуля на шумных topology-каналах; BC выводит политику из мёртвой зоны $16\times16$, после чего PPO находит стратегию выше уровня учителя.

			\item \textbf{CNN и ViT при равном бюджете параметров ($\approx1.25$М) статистически неразличимы} на $16\times16$ — ограничивает качество пайплайн BC+PPO, а не архитектура.

			\item \textbf{Стабильность:} CV нейросетей $\approx0.64$--$0.67$ против $0.90$ у эвристики ($-29\%$); инференс в $10\times$ быстрее.
		\end{itemize}
	\end{frame}
